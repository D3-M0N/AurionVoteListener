	...
	public static synchronized boolean Voted(String player, int totalvotes, long now) {
		...
		try {
			if (SQLTYPE.equals("MySQL")) {
				AurionsVoteListener.GetInstance();
				sql = connection.prepareStatement("INSERT INTO `" + AurionsVoteListener.dbPrefix + TableTotal + "` (`IGN`, `votes`, `lastvoted`) VALUES ('" + player + "', " + totalvotes + ", " + now + ") ON DUPLICATE KEY UPDATE `votes` = " + totalvotes
						+ ", `lastvoted` = " + now + ", `IGN` = '" + player + "';");
				sql.executeQuery();
	...
	public static void offline(String username, String serviceName, String timeStamp, String address) {
		...
		try {
			if (SQLTYPE.equals("MySQL")) {
				...
				AurionsVoteListener.GetInstance();
				sql = connection
						.prepareStatement("INSERT OR IGNORE INTO `" + AurionsVoteListener.dbPrefix + TableQueue + "` (`IGN`, `service`, `timestamp`, `ip`) VALUES ('" + username + "', '" + serviceName + "', '" + timeStamp + "', '" + address + "');");
				sql.execute();
			}
			sql.close();
		} catch (SQLException e) {
